...
object InputParser {
  ...
  private def getInputInstructions(inputLocation: String): List[String] = File(inputLocation).lines.toList
  ...
  private def parseInputsIntoInstructions(input: List[String]): Try[Limits] = getLimitsFromInput(input).fold(
    limitError => Failure(limitError),
    limits => Success(limits)
  )

  private def getLimitsFromInput(input: List[String]): Either[InputFormatException, Limits] = input.headOption match {
    case None => Left(new InputFormatException("Input file must start with 2 int indicating the garden size with this format 'x y'"))
